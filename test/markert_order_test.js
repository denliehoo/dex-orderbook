...
contract("Dex", accounts => {
    ...
    it("Market orders should be filled until the order book is empty", async () => {
        ...
        let orderbook = await dex.getOrderBook(web3.utils.fromUtf8("LINK"), 1); //Get sell side orderbook
        assert(orderbook.length == 1, "Sell side Orderbook should have 1 order left"); //(from the prev test)
        ...
        //Buyer should have 15 more link after, even though order was for 50 since the boook only has 15 
    ...
    it("The eth balance of the buyer should decrease with the filled amount", async () => {
        ...
        await dex.createMarketOrder(0, web3.utils.fromUtf8("LINK"), 1); //account[0] mdde this order
